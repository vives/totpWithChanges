...
public class TOTPAuthenticator extends AbstractApplicationAuthenticator implements LocalApplicationAuthenticator {
    ...
    protected void initiateAuthenticationRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        String loginPage = ConfigurationFacade.getInstance().getAuthenticationEndpointURL()
                .replace(TOTPAuthenticatorConstants.DEFAULT_LOGIN_ENDPOINT,
                        TOTPAuthenticatorConstants.LOGIN_PAGE);
        ...
        try {
            ...
            boolean isTOTPEnabled = TOTPAccessController.isTOTPEnabledForLocalUser(username, context);
        ...
        try {
            ...
            if (!context.getTenantDomain().equals(TOTPAuthenticatorConstants.SUPER_TENANT_DOMAIN)) {
                if (Boolean.parseBoolean((context.getProperty(TOTPAuthenticatorConstants.AUTHENTICATION_POLICY_ENABLED))
                        .toString())) {
                    multiFactorAuthenticationEventListener = new MultiFactorAuthenticationEventListener();
            ...
            if (token != null) {
                try {
                    ...
                    if (!TOTPTokenVerifier.isValidTokenLocalUser(tokenValue, username, context)) {
    ...
    public void executeAccountLockingPolicy(AuthenticationContext context, String username,
        ...
        if (!context.getTenantDomain().equals(TOTPAuthenticatorConstants.SUPER_TENANT_DOMAIN)) {
            if ((Boolean.parseBoolean(context.getProperty(TOTPAuthenticatorConstants.AUTHENTICATION_POLICY_ENABLED).
                    toString()))
                    && Boolean.parseBoolean(context.getProperty(TOTPAuthenticatorConstants.
                    AUTHENTICATION_LOCKING_POLICY_ENABLED).toString())) {
