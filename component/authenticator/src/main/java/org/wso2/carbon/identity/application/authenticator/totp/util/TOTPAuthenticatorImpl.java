...
public final class TOTPAuthenticatorImpl {
	 ...
	 */
	private static final Logger LOGGER = Logger.getLogger(TOTPAuthenticatorImpl.class.getName());
	...
	private TOTPReseedingSecureRandom secureRandom =
			new TOTPReseedingSecureRandom(getRandomNumberAlgorithm(), getRandomNumberAlgorithmProvider());

	public TOTPAuthenticatorImpl(TOTPAuthenticatorConfig config) {
		if (config == null) {
	...
	public TOTPAuthenticatorKey createCredentials() {
		...
		return new TOTPAuthenticatorKey(generatedKey, validationCode, scratchCodes);
	}
	...
	private List<Integer> calculateScratchCodes(byte[] buffer) {
		...
		while (scratchCodes.size() < SCRATCH_CODES) {
			byte[] scratchCodeBuffer =
					Arrays.copyOfRange(buffer, SECRET_BITS / 8 + BYTES_PER_SCRATCH_CODE * scratchCodes.size(),
	...
	private int calculateScratchCode(byte[] scratchCodeBuffer) {
		if (scratchCodeBuffer.length < BYTES_PER_SCRATCH_CODE) {
			throw new IllegalArgumentException(
					String.format("The provided random byte buffer is too small: %d.", scratchCodeBuffer.length));
		}
		int scratchCode = 0;
	...
	public boolean authorize(String secret, int verificationCode) throws TOTPAuthenticatorException {
		return authorize(secret, verificationCode, new Date().getTime());
	}

	public boolean authorize(String secret, int verificationCode, long time) throws TOTPAuthenticatorException {
