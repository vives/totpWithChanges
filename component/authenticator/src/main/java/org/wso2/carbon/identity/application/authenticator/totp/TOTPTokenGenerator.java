...
public class TOTPTokenGenerator {
        ...
        if (username != null) {
            ...
            try {
                ...
                if (userRealm != null) {
                    ...
                    String firstName = userRealm.getUserStoreManager().getUserClaimValue(username,
                    ...
                    if (TOTPAuthenticatorConstants.BASE32.equals(encoding)) {
                        ...
                        secretKeyByteArray = code64.decode(secretKey);
                    }

                    token = getCode(secretKeyByteArray, getTimeIndex(context));
                    sendNotification(username, firstName, Long.toString(token), email);
    ...
    private static void sendNotification(String username, String firstName, String token, String email) throws TOTPException {
        ConfigurationContext configurationContext = TOTPDataHolder.getInstance().getConfigurationContextService().
        ...
        if (configurationContext.getAxisConfiguration().getTransportsOut()
            ...
            if (config.getProperties().containsKey(TOTPAuthenticatorConstants.EMAIL_TEMPLATE_NMAME)) {
                emailTemplate = config.getProperty(TOTPAuthenticatorConstants.EMAIL_TEMPLATE_NMAME);
                try {
                    emailNotification = NotificationBuilder.createNotification("EMAIL", emailTemplate,
                            emailNotificationData);
                } catch (IdentityMgtServiceException e) {
                    log.error("Error occurred while creating notification from email template : " + emailTemplate,
                            e);
